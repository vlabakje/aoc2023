...
class Grid:
    ...
    def max_start(self):
        ...
        for x in range(self.width):
            top = max(top, self.simulate(Beam(x, 0, Directions.SOUTH)))
            top = max(top, self.simulate(Beam(x, self.height-1, Directions.NORTH)))
        for y in range(self.height):
            top = max(top, self.simulate(Beam(0, y, Directions.EAST)))
            top = max(top, self.simulate(Beam(self.width-1, y, Directions.WEST)))
        return top
